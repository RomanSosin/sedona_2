...
var gulp = require("gulp"); /* Подключаем gulp, функцией require в nodejs*/
var del = require("del");
...
в браузере */
var less = require("gulp-less"); /* Подключаем установленную через npm i gulp-less зависимость */
var postcss = require("gulp-postcss"); /* Подключаем префиксы для старых браузеров */
var autoprefixer = require("autoprefixer"); /* Подключаем автопрефиксер для postcss */
...
gulp.task("css", function () { /* конвертация препроцессорного кода и минификация CSS */
  return gulp.src("source/less/style.less")
    ...
    .pipe(sourcemap.init())
    .pipe(less()) /* Запускаем функцию less */
    .pipe(postcss([ /* Запускаем postcss с внутренним автопрефиксиром */
      autoprefixer()
    ]))
    .pipe(csso()) /* Минимизатор после всех преобразований */
...
gulp.task("images", function () { /* Оптимизируем изображение */
  return gulp.src("source/img/**/*.{png,jpg,svg}") /* Описываем контент с которым будем работать */
    ...
    .pipe(gulp.dest("source/img")); /* Сохраняем получившийся контент в папку css */
});
...
gulp.task("copy", function () { /* Копируем файлы в отдельную папку build */
  return gulp.src([
    ...
    base: "source" /* Укажем корень для копирования папку source */
  })
  .pipe(gulp.dest("build"));
});
